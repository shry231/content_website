...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (shiftImgEl && heartOverlay && nextShiftImgEl) {
    ...
    if (doubleTapHint && hintText) {
      ...
      // trigger blink (runs twice) then remove blink class but keep visible
      hintText.classList.add('blink');
      ...
      const onOverlayClick2 = async (e) => {
        ...
        try { applyClickNudge(0.75, 'comments2'); } catch (e) { /* ignore */ }
          ...
          if (stage8El) {
            ...
            // hide static bottom decoration so the scroller ends at the end of stage8
            try { const sb = document.getElementById('staticBottom'); if (sb) sb.style.display = 'none'; } catch (e) {}
  ...
  async function runFinalSequence(revealTarget) {
    ...
    overlay.style.display = 'none';
    overlay.setAttribute('aria-hidden', 'true');
    // Hide the double-tap hint after a short delay so it lasts a bit longer
    const hintElLater = document.getElementById('doubleTapHint');
    if (hintElLater) {
      try {
        setTimeout(() => { hintElLater.classList.remove('visible'); }, 1800);
      } catch (e) { hintElLater.classList.remove('visible'); }
    }
